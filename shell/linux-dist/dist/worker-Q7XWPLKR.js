...
const workerLog = (msg) => {
    const full = "[W:" + (self.name || '?') + "] " + msg;
    postMessage({ type: "log", msg: full });
    // Always forward to HTTP debug log (fire-and-forget; server may not be running)
    try { fetch('/log', { method: 'POST', body: full, keepalive: true }).catch(()=>{}); } catch(e){}
};
const dbglog = workerLog;
        ...
        imports: {
                    ...
                    instance = new WebAssembly.Instance(module, {
                        ...
                        linux: {
                            syscall: (nr, arg0, arg1, arg2, arg3, arg4, arg5) => {
                                ...
                                // Log critical fork/pipe syscalls to debug server
                                const IS_PIPE2   = (nr === 59);
                                const IS_EXECVE  = (nr === 221);
                                const IS_VFORK_CLONE = (nr === 220 && arg2 === 0x4111);
                                if (IS_PIPE2 || IS_VFORK_CLONE || IS_EXECVE) {
                                    dbglog('PRE sc nr=' + nr + ' a0=0x' + arg0.toString(16) + ' a1=0x' + arg1.toString(16) + ' a2=0x' + arg2.toString(16));
                                }
                                ...
                                // Intercept SYS_clone for fork() and vfork() — wasm-stubs.c path for asyncified
                                // binaries (e.g. dropbear built with wasm-stubs.c instead of wasm_fork.c).
                                // fork()  → clone(SIGCHLD=17, 0)          arg0=17
                                // vfork() → clone(CLONE_VFORK|CLONE_VM|SIGCHLD, 0)  arg0=0x4111=16657
                                // Match any clone where: no new stack (arg1=0), not a thread (no CLONE_THREAD=0x10000),
                                // and SIGCHLD (0x11) is in the low byte of flags.
                                if (nr === 220 && arg1 === 0 && !(arg0 & 0x10000) && (arg0 & 0xFF) === 17 && instance?.exports?.asyncify_get_state) {
                                    const bufSize = 131072; // 128 KB
                                    const bufPtr = memory.buffer.byteLength - bufSize;
                                    const retPtr = bufPtr - 4; // 4 bytes just before the buffer, safe from asyncify data
                                    const state = instance.exports.asyncify_get_state();
                                    console.log('[FIX:SYS_clone] nr=220 arg0=0x' + arg0.toString(16) + ' state=' + state + ' bufPtr=0x' + bufPtr.toString(16));
                                    if (state === 0) {
                                        ...
                                        h[(bufPtr >> 2) + 1] = bufPtr + bufSize; // end of data
                                        pendingFork = { bufPtr, retPtr, vfork: false };
                                        instance.exports.asyncify_start_unwind(bufPtr);
                                        return 0;
                                    } else if (state === 2) {
                                        // Phase 3 (Rewinding): stop rewind, return fork result written by handler
                                        instance.exports.asyncify_stop_rewind();
                                        return new Int32Array(memory.buffer)[retPtr >> 2];
                                    }
                                ...
                                if (IS_PIPE2 || IS_VFORK_CLONE || IS_EXECVE) {
                                    dbglog('POST sc nr=' + nr + ' ret=' + ret + (IS_VFORK_CLONE ? ' (VFORK_CLONE)' : IS_PIPE2 ? ' (PIPE2)' : ' (EXECVE)'));
                                }
            ...
            call() {
                ...
                for (;;) {
                    try {
                        ...
                        dbglog('call() FATAL ERROR: ' + String(error) + ' | pendingFork=' + !!pendingFork + ' asyncify_state=' + (instance?.exports?.asyncify_get_state?.() ?? 'N/A'));
            ...
            switch_entry(fn, arg) {
                ...
                if (parent_memory && arg > 0 && arg < parent_memory.buffer.byteLength - 64) {
                    ...
                    dbglog('switch_entry fn=' + fn + ' arg=' + arg);
                }
                ...
                call_entry = () => {
                    ...
                    dbglog('call_entry calling fn=' + fn + ' arg=' + arg);
                    f(arg);
                    workerLog('call_entry fn=' + fn + ' returned (no exit!)');                        dbglog('call_entry fn=' + fn + ' returned (no exit!) ERROR');                    console.warn("thread entrypoint reached the end without exiting");
                };
            },
            // signal handling:
...
self.onmessage = (event) => {
    ...
    dbglog('WORKER START fn=' + fn + ' has_parent_user=' + (parent_user_module != null) + ' fork=' + (fork_bufPtr != null));
    ...
    try {
        ...
        if (error === HALT_KERNEL)
            return;
        // Fatal kernel error — attempt sys_exit(1) so the kernel can complete
        // vfork_done before this Worker dies, preventing the parent from hanging
        // forever in Atomics.wait32.
        const errStr = String(error);
        workerLog('FATAL: ' + errStr);
        try {
            // AArch64 exit_group = syscall 94; sys_exit = 93
            instance.exports.syscall(94, 1, 0, 0, 0, 0, 0);
        } catch(e2) {
            workerLog('FATAL: sys_exit also threw: ' + String(e2));
        }
        throw error;
    }
};
