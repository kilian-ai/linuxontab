import { Bytes, Struct, U32BE, U64BE, U8, } from "./bytes.js";
...
function align(bytes, alignment) {
    ...
    if (offset !== 0) {
        for (let j = 0; j < alignment - offset; j++)
            bytes.alloc(U8);
    }
}
export function generate_devicetree(tree, { memory_reservations = [], boot_cpu_id = 0, } = {}) {
    ...
    function walk_tree(node, name) {
        ...
        assert(encodedName.byteLength <= NODE_NAME_MAX_LEN, `property name too long: ${name}`);
        bytes.append(encodedName);
        bytes.alloc(U8).value = 0;
        align(bytes, 4);
        const children = Object.entries(node).filter(([, value]) => typeof value === "object" && value?.constructor === Object);
        const properties = Object.entries(node).filter(([, value]) => !(typeof value === "object" && value?.constructor === Object));
        for (const [name, prop] of properties) {
            align(bytes, 4);
            bytes.alloc(U32BE).value = FDT_PROP;
            const property = bytes.alloc(Property);
            assert(new TextEncoder().encode(name).byteLength <= PROPERTY_NAME_MAX_LEN, `property name too long: ${name}`);
    ...
    for (const [str, refs] of Object.entries(strings)) {
        ...
        for (const ref of refs)
            ref.value.nameoff = offset - begin_dt_strings;
    }
