#!/bin/sh
# Recipe: libXrandr — X RandR extension client library

NAME="libXrandr"
VERSION="1.5.4"
DESCRIPTION="X RandR extension client library"
SOURCE_URL="https://www.x.org/archive/individual/lib/libXrandr-${VERSION}.tar.gz"
SOURCE_SHA256=""
...
build() {
    ...
    XPROTO_SRC="/tmp/lot-build/libXrandr-xorgproto"
    ...
    for dep in libX11 libXext libXrender libXfixes; do
    ...
    DEPS_PREFIX="/tmp/lot-build/libXrandr-deps-prefix"
    rm -rf "$DEPS_PREFIX"
    ...
    printf '%s\n' \
        ...
        'if [ "$1" = "--libs" ]; then echo "-L'"$DEPS_PREFIX"'/lib -lXrender -lXext -lX11"; exit 0; fi' \
        'exit 0' \
        > "$SRC/pkg-config"
    ...
    ./configure \
        ...
        LIBS="$CRT1 -lc -lm $BUILTINS -lXrender -lXext -lX11"
