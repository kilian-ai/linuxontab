#!/bin/sh
# Recipe: libXpm — X pixmap library (needed by libXaw → xterm)

NAME="libXpm"
VERSION="3.5.17"
DESCRIPTION="X pixmap library"
SOURCE_URL="https://www.x.org/archive/individual/lib/libXpm-${VERSION}.tar.gz"
SOURCE_SHA256=""
...
build() {
    ...
    XPROTO_SRC="/tmp/lot-build/libXpm-xorgproto"
    ...
    DEPS_PREFIX="/tmp/lot-build/libXpm-deps-prefix"
    rm -rf "$DEPS_PREFIX"
    ...
    printf '%s\n' \
        ...
        'if [ "$1" = "--modversion" ]; then echo "1.8.10"; exit 0; fi' \
    ...
    # Library only — the sxpm/cxpm tools want getopt/pipes we don't need.
    make -j4 -C src LIBS=""
